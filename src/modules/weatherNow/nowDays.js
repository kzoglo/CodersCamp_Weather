...
function render(x) {
  $.get('/CodersCamp_Weather/modules/weatherNow/nowDays.mst', function(
    template
  ) {
...
function createWindChar(x) {
  $('#chartContainer').html('');
  $.get('/CodersCamp_Weather/modules/weatherNow/wind.mst', function(template) {
    const result = Mustache.render(template, x.list[0]);
...
function createRainChar(x) {
  $('#chartContainer').html('');
  $.get('/CodersCamp_Weather/modules/weatherNow/rain.mst', function(template) {
