...
import {
    changeToPresentDay
} from './modules/weatherNow/nowDays.js';
import {
    changeToOneDay
} from './modules/weatherOneDay/oneDay.js';
...
let doneTypingInterval = 500; // Time in ms (.5 second)
...
searchCity.addEventListener('keyup', e => {
    ...
    if (value) {
        ...
        autocompleteList.classList.remove('autocomplete-list-overflow', 'autocomplete-list-visible');
    }
});
...
async function getData() {
    const all = await fetch('../src/assets/cityList/cityList.json');
...
function renderCities(value, jsonArray) {
    ...
    let html = cities.map(city => {
        ...
        return `
            <li class="autocomplete-item" data-city_id="${id}">${name}, ${country}</li>
        `;
    });
    ...
    if (cities.length > 13) autocompleteList.classList.add('autocomplete-list-overflow');
